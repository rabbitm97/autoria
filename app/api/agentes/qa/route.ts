...
export interface QAResult {
  ...
  itens: QAItem[];
  recomendacao: string; // parágrafo gerado por Claude
  analisado_em: string;
}
...
export async function POST(req: NextRequest) {
  ...
  // ── Texto checks
  if (!texto.trim()) {
    itens.push({ categoria: "texto", status: "erro", mensagem: "Nenhum texto de manuscrito encontrado. Execute o parse." });
    ...
    itens.push({ categoria: "texto", status: "aviso", mensagem: `Manuscrito curto (${palavras.toLocaleString("pt-BR")} palavras). Considere expandir para publicação comercial.` });
  } else {
    itens.push({ categoria: "texto", status: "ok", mensagem: `${palavras.toLocaleString("pt-BR")} palavras — tamanho adequado para publicação.` });
  }
  ...
  if (!sinopseCurta) {
    itens.push({ categoria: "metadados", status: "aviso", mensagem: "Sinopse curta não preenchida." });
  } else {
    itens.push({ categoria: "metadados", status: "ok", mensagem: "Sinopse curta presente." });
  }

  if (!sinopseLonga) {
    itens.push({ categoria: "metadados", status: "aviso", mensagem: "Sinopse longa não preenchida." });
  } else {
    itens.push({ categoria: "metadados", status: "ok", mensagem: "Sinopse longa presente." });
  }

  if (palavrasChave.length < 3) {
    itens.push({ categoria: "metadados", status: "aviso", mensagem: `Poucas palavras-chave (${palavrasChave.length}). Recomendado: 7–10.` });
  } else {
    itens.push({ categoria: "metadados", status: "ok", mensagem: `${palavrasChave.length} palavras-chave definidas.` });
  }

  if (!fichaCatalografica) {
    itens.push({ categoria: "metadados", status: "aviso", mensagem: "Ficha catalográfica não preenchida." });
  } else {
    itens.push({ categoria: "metadados", status: "ok", mensagem: "Ficha catalográfica presente." });
  }
  ...
  // ── Diagramação checks
  if (!temPdf) {
    itens.push({ categoria: "diagramacao", status: "aviso", mensagem: "PDF ainda não gerado. Gere o PDF antes de publicar." });
  } else {
    itens.push({ categoria: "diagramacao", status: "ok", mensagem: "PDF gerado com sucesso." });
  }
  ...
  const claudeRes = await anthropic.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Você é um consultor editorial. Analise o relatório QA abaixo de um projeto de publicação de livro e escreva UMA recomendação final em 2-3 frases em português. Seja direto e útil.

Título: ${titulo || "Não definido"}
Palavras no manuscrito: ${palavras.toLocaleString("pt-BR")}
Score: ${score}/100
...
Escreva apenas a recomendação, sem título, sem listas.`,
    }],
  });

  const recomendacao = (claudeRes.content[0] as { text: string }).text.trim();
